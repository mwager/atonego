...
// mwager atonego we do not need this in the global namespace
// window.Zepto = Zepto
  ...
  $(document).ready(function(){
    ...
    $(document.body)
      ...
      .bind('touchmove', function(e){
        ...
        // atonego mwager
        // prevent scrolling if swipe detected
        // -> https://github.com/madrobby/zepto/commit/d945672b89be7957b268e01df1dc6425de5932b1
        // (WE DO NOT USE SWIPE EVENTS)
      ...
      .bind('touchend', function(e){
        ...
        if ((touch.x2 && Math.abs(touch.x1 - touch.x2) > 30) ||
          ...
          swipeTimeout = setTimeout(function() {
            if(!touch.el) return; // atonego - strange errors in WebView via phonegap
        ...
        // normal tap
          ...
          tapTimeout = setTimeout(function() {
            if(!touch) return; // atonego - strange errors in WebView via phonegap
            if(!touch.el) return; // atonego
            if(!$) return; // atonego
            if(!$.Event) return; // atonego
...
// zepto-ghostclicks
// https://github.com/ConradIrwin/zepto-ghostclick/blob/master/zepto.ghostclick.js
/*global Zepto, window*/
...
/*
IMMER NOCH PROBLEME:
klick auf listen-einstellungen -> fokus in textfeld auf nächster view )-:
DAMNIT!
*/
...
(function($) {
    ...
    var DEBUG = true;
    var  touch = 'touchstart' in window;
    var FastButton;

    // Busting Ghost Clicks
    // Adding the onclick handler back to the button introduces one last ugly
    // problem. When you tap the button, a click event will still be fired
    // 300ms later. Now we run the risk of the click handler executing twice.
    // This can be easily resolved by calling preventDefault on the touchstart
    // event. Calling preventDefault on touchstart events will stop clicks and
    // scrolling from occurring as a result of the current tap.
    // We wanted users to be able to scroll even if they started their scroll
    // on a button, so we didn’t consider this an acceptable solution.
    // ==> What we came up with to deal with these ghost clicks was called a
    // click buster. What we do is simply add a click event listener to the
    // BODY!, listening on the capture phase. When our listener is invoked,
    // we try to determine if the click was as a result of a tap that we
    // already handled, and if so we call preventDefault and stopPropagation on it.
    var clickbuster = {
    ...
    // touchmove and touchend events. Calling stopPropagation guarantees
    // that other behaviors don’t get a chance to handle the same click event.
    FastButton.prototype.onTouchStart = function (event) {
        if(DEBUG) {
            log('=== TOUCH START: ' + event.touches);
        }
    ...
    $.extend($.fn, {
        onTap: function (selector, handler) {
